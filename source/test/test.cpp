...
int pack_encrypt(std::string &params_str) {
    ...
    if ((params_vec.size() != (8 + file_num)) ||
        (file_num > IPEDUParams::MAX_RES_INPUT_NUM)) {
        std::cout << "pack_encrypt params is invalid." << std::endl;
        return IFLYPEDU_ERROR_GENERAL;
    }
    ...
    for (int index = 0; index < file_num; index++) {
        function_params.input_res_paths[index] =
            (char *)params_vec[2 + index].c_str();
    }
    memcpy(model_params.model_name, params_vec[2 + file_num].c_str(),
           strlen(params_vec[2 + file_num].c_str()) + 1);
    model_params.model_version = std::atoi(params_vec[3 + file_num].c_str());
    model_params.model_type = static_cast<IPEDUModelType>(
        std::atoi(params_vec[4 + file_num].c_str()));
...
int decrypt_unpack(std::string &params_str) {
    ...
    if ((params_vec.size() < 3) ||
        (params_vec.size() > 2 + IPEDUParams::MAX_RES_INPUT_NUM)) {
    ...
    printf("MAX_IPUTY_SIZE : [N:%d, C:%d, H:%d, W:%d]\n", model_params.n,
           model_params.c, model_params.h, model_params.w);
...
int main(int argc, char **argv) {
    ...
    if (3 != argc) {
        ...
        printf("[TOOL_TYPE = PACK_ENCRYPT] "
               "\"conf_path:file_num:file1_path:fiel2_path:...:model_name:"
               "model_version:model_type:magic_num:key:output_path\"; \n");
        printf("[TOOL_TYPE = DECRYPT_UNPACK] "
               "\"packed_path:key:output1_path:output2_path:...\"; \n");
        return -1;
    }
    ...
    if (function_map.find(mode) == function_map.end()) {
        std::cout << "Can not find " << mode << " in the tool_type_map"
                  << std::endl;
        return -1;
    }
